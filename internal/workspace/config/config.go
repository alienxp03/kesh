...
func validateConfig(config *Config, filePath string, homeDir string) error {
	...
	if err := validateTerminalSessionName(config.Terminal.SessionName); err != nil {
		return fmt.Errorf("invalid config in %s: terminal.session_name %w", filePath, err)
	}
...
func validateTerminalSessionName(template string) error {
	...
	for {
		...
		if !isTerminalSessionNameReference(reference) {
...
func isTerminalSessionNameReference(reference string) bool {
...
func validateTerminalNode(node *yaml.Node, filePath string) error {
	if node.Kind != yaml.MappingNode {
		return fmt.Errorf("invalid config in %s: terminal must be an object", filePath)
	}
	for i := 0; i < len(node.Content); i += 2 {
		key := node.Content[i].Value
		switch key {
